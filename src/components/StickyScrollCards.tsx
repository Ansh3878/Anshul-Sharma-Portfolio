...
function StickyScrollCard({
  ...
  const cardContent = (
    ...
    >
      ...
      <div className="project-card-content flex flex-col p-6 pt-0 -mt-10 relative z-10 h-[180px]">
        <div className="project-card-heading flex items-center gap-3 mb-3">
          <h3 className="project-card-title text-2xl font-bold text-white tracking-tight">{title}</h3>
          {tagline && (
            <span className="project-card-tagline text-[10px] font-medium px-2.5 py-1 rounded-full bg-white/10 text-white/80 uppercase tracking-widest">
              {tagline}
            </span>
          )}
        </div>
        ...
        {tags && (
          <div className="project-card-tags flex flex-wrap gap-2 mt-auto">
            {tags.map((tag) => (
              <span key={tag} className="project-card-tag text-[10px] font-mono uppercase tracking-[0.1em] text-zinc-500 bg-black/40 px-2.5 py-1 rounded border border-white/5">
                {tag}
              </span>
            ))}
          </div>
  ...
  return (
    <div
      className="project-card-stage sticky top-0 flex h-screen items-center justify-center"
      style={{ "--project-card-offset": `${i * 22}px` } as CSSProperties}
    >
      {href ? (
        <a href={href} target="_blank" rel="noopener noreferrer" className="project-card-link block focus:outline-none focus-visible:ring-2 focus-visible:ring-white/50 rounded-2xl">
          {cardContent}
        </a>
      ) : (
        cardContent
  ...
  return (
    ...
    >
      <main
        ref={container}
        className={cn(
          "project-stack-container relative flex w-full flex-col items-center justify-center pb-[80vh] pt-[32vh]",
          className
        )}
      >
        {/* Hint label */}
